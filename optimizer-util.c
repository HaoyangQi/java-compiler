...
{
    if (r && r->type == IR_ASN_REF_DEFINITION && r->def->type == DEFINITION_VARIABLE)
    {
        return r->def;
    }
...
instruction* optimizer_phi_place(basic_block* node, definition* variable, size_t id)
{
    ...
    // reserve operand space: number of incoming edge
...
/**
 * Append Operand To A PHI function
 *
 * PHI(V) = PHI(V) union {assignment}
 *
 * assignment instruction has form: V <- _
 * and it will be inserted to PHI(V)
 * and PHI(V) is inserted at the beginning of the node
*/
bool optimizer_phi_set_operand(basic_block* node, size_t idx, instruction* assignment)
{
    if (!assignment) { return false; }

    instruction* phi = optimizer_phi_locate(node, assignment->lvalue->def);

    if (!phi || idx >= phi->operand_phi.num || phi->operand_phi.arr[idx])
    {
        fprintf(stderr, "TODO error: phi index invalid or the operand is already set.\n");
        return false;
    }

    phi->operand_phi.arr[idx] = assignment;
    return true;
}
